 ...
 */
export const releaseDepositFunction = functions.https.onCall(async (request) => {
 ...
 */
export const releasePartialDepositFunction = functions.https.onCall(async (request) => {
  const { auth, data } = request;
 ...
 */
export const holdDepositFunction = functions.https.onCall(async (request) => {
  const { auth, data } = request;
 ...
 */
async function notifyUserAboutDeposit(
  userId: string,
  status: string,
  amount: number,
  reason: string
 ...
 */
export function validateDepositOperations(depositData: any): boolean {
  if (!depositData) {
