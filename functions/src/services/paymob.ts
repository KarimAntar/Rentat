...
export class PaymobFunctionsService {
  ...
  private readonly baseUrl: string = 'https://accept.paymob.com/api';
  ...
  public static getInstance(): PaymobFunctionsService {
    if (!PaymobFunctionsService.instance) {
  ...
  // Create payment key
  public async createPaymentKey(data: {
    amount: number;
    currency: string;
    orderId: number;
    billingData: {
  ...
  // Void transaction
  public async voidTransaction(transactionId: string): Promise<{ success: boolean }> {
    try {
      const authToken = await this.authenticate();
